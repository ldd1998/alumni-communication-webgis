...
*/
    ...
    public  ResultVo userReg(@RequestBody UserInfo userInfo, HttpSession session) {
        ...
        Login login = new Login().setMobilephone(mobilephone);//这里和链式写法有关系
        //查询账号是否已经注册
        Login userIsExist = loginService.userLogin(login);
        if (!StringUtils.isEmpty(userIsExist)){//用户账号已经存在
...
//        if (rel.equalsIgnoreCase(vercode)) {//验证码正确
        if (true) {//验证码正确
            ...
            //新注册用户存入默认头像、存入默认签名
            userInfo.setUserid(userid).setPassword(passwords).setUimage("/pic/head/2.png").
            ...
            if (integer==1 && integer1==1){
                ...
                session.setAttribute("USER",userInfoService.LookUserinfo(userid));
    ...
    public ResultVo userLogin(@RequestBody Login login, HttpSession session){
        String account=login.getUsername();//获取用户名
        String password=login.getPassword();//获取密码
        String vercode=login.getVercode();//获取验证码
        ...
        try {
            ...
            session.setAttribute("USER",userInfoService.LookUserinfo(login1.getUserid()));
